...
class ADHA_oneclass(Dataset):
	...
	def __getitem__(self, idx):
		...
		for i in range(10):
			if os.path.isfile(self.input_path + '/' + sample[0] + '/' + sample[1] + '/' + 'of' + '/' + str(i*8) + '_horizontal' + '.jpg'):
				img = cv2.imread(self.input_path + '/' + sample[0] + '/' + sample[1] + '/' + 'of' + '/' + str(i*8) + '_horizontal' + '.jpg')
			...
			if os.path.isfile(self.input_path + '/' + sample[0] + '/' + sample[1] + '/' + 'of' + '/' + str(i*8) + '_vertical' + '.jpg'):
				img = cv2.imread(self.input_path + '/' + sample[0] + '/' + sample[1] + '/' + 'of' + '/' + str(i*8) + '_vertical' + '.jpg')
				x = cv2.resize(img, (224, 224), interpolation=cv2.INTER_CUBIC)
				x = cv2.cvtColor(x, cv2.COLOR_BGR2GRAY)
				img = Image.fromarray(x)
				transformed_img = self.transform(img)
...
def record_info(info, mode):
	...
	if mode == 'test':
		result = (
			...
			'Prec@5_act {top5_act}'
			'Prec@1_adv {top1_adv} '
			'Prec@5_adv {top5_adv}'.format(epoch=info['Epoch'],step=info['Step'],
